...
const FormTitle: React.FunctionComponent<FormTitleType> = (props) => {
  ...
  function renderOptions() {
    return data.map((item, index) => {
      return (
        <Label check key={index} style={{ marginRight: "2rem" }} className="mb-4">
          <Input type="checkbox" onChange={changeValue} style={{ border: "1px solid #FAC228" }}value={item}checked={value === item} />
          {" "}{item}
        </Label>
      );
    });
